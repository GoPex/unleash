...
func bitbucketPush (c *gin.Context){
    ...
    if err != nil {
        c.JSON(http.StatusUnauthorized, gin.H{"status": "Bitbucket OAuth failed !"})
        return
    }
    ...
    if c.BindJSON(&json) == nil {
        for _, change := range json.Push.Changes {
            // Launch the build in background
            go BuildAndPushFromRepository(json.Repository.Links.HTML.Href, json.Repository.FullName, change.New.Name, change.New.Target.Hash, token.AccessToken)
        }
        // Render status OK (200)
